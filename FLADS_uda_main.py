...
if args.EXP.AUGMENTATION != None and args.EXP.AUGMENTATION != 'None':
    ...
    if "LADS" in args.EXP.AUGMENTATION or 'Directional' in args.EXP.AUGMENTATION:
        # wsj has changed
        # add optuna None
        augment = getattr(methods.augmentations, args.EXP.AUGMENTATION)(args, None, few_target_domain_features, removed_style_img_embs, train_features, train_labels, train_groups, train_domains, train_filenames, bias_correction.text_embeddings, val_features, val_labels, val_groups, val_domains)
    else:
