...
def objective(trial):
    ...
    # created by wsj: get_few_target_domain_index
    # 1 image per class, 200 total
    ...
    # 5 image from class folders which include 5 more images, 178*5=890 total
    # def get_few_target_domain_index(test_labels):
    #     test_labels = test_labels[:3047]
    #     num_classes = len(set(test_labels))
    #     few_target_domain_index = []
    ...
    #             few_target_domain_index.extend(class_index[0:5])
    #     return few_target_domain_index
    ...
    # load style-removed img embds
    style_removed_save_dict = torch.load('./embeddings/CUB/clip_openai_Vit-L_14_remove_style.pt')
    removed_style_img_embs = style_removed_save_dict["removed_style_features"]
    ...
    few_target_domain_features,test_features = test_process(test_features,few_target_domain_index)
    few_target_domain_labels,test_labels = test_process(test_labels,few_target_domain_index)
    few_target_domain_groups,test_groups = test_process(test_groups,few_target_domain_index)
    few_target_domain_domains,test_domains = test_process(test_domains,few_target_domain_index)
    few_target_domain_filenames,test_filenames = test_process(test_filenames,few_target_domain_index)
    ...
    if args.EXP.AUGMENTATION != None and args.EXP.AUGMENTATION != 'None':
        ...
        if "LADS" in args.EXP.AUGMENTATION or 'Directional' in args.EXP.AUGMENTATION:
            ...
            # optuna
            weight_dict = {
                'weight_text_direction': trial.suggest_categorical("weight_text_direction", [0.01, 0.03, 0.1, 0.3, 1]),
                "weight_image_direction": trial.suggest_categorical("weight_image_direction", [0.01, 0.03, 0.1, 0.3, 1]),
                "weight_text_l1": trial.suggest_categorical("weight_text_l1", [0.01, 0.03, 0.1, 0.3, 1]),
                "weight_image_l1": trial.suggest_categorical("weight_image_l1", [0.01, 0.03, 0.1, 0.3, 1]),
                "weight_cls_consist": trial.suggest_categorical("weight_cls_consist", [0.01, 0.03, 0.1, 0.3, 1]),
                "weight_mmd": trial.suggest_categorical("weight_mmd", [1, 3, 10, 30, 50, 100])
            }

            augment = getattr(methods.augmentations, args.EXP.AUGMENTATION)(args, weight_dict, few_target_domain_features, removed_style_img_embs, train_features, train_labels, train_groups, train_domains, train_filenames, bias_correction.text_embeddings, val_features, val_labels, val_groups, val_domains)
        else:
