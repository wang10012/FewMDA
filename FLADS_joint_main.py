...
#         few_indices = indices[:len(indices)//2]
#         few_target_domain_index.extend(few_indices)
...
if args.EXP.AUGMENTATION != None and args.EXP.AUGMENTATION != 'None':
    ...
    # wsj add
    # train_features = np.concatenate((train_features,few_target_domain_features),axis=0)
    # train_labels = np.concatenate((train_labels,few_target_domain_labels),axis=0)
    # train_domains = np.concatenate((train_domains,few_target_domain_domains),axis=0)
    # train_groups = np.concatenate((train_groups,few_target_domain_groups),axis=0)
    # train_filenames = np.concatenate((train_filenames,few_target_domain_filenames),axis=0)
